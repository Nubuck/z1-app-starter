  ...
  return [
      ...
      async ({ getState }, dispatch, done) => {
        ...
        const nextStatus = t.not(t.or(t.eq(size, 'lg'), t.eq(size, 'xl')))
          ? t.eq(status, NAV_STATUS.INIT)
            ...
            : status
          : NAV_STATUS.INIT
        if (t.or(t.not(t.eq(status, nextStatus)), t.not(t.eq(size, navSize)))) {
          dispatch(
            mutations.navChange({
    ...
    fx(
      ...
      async ({ getState }, dispatch, done) => {
        ...
        // mode
        const nextMode = t.isZeroLen(t.concat(primary.items, primary.actions))
          ? NAV_MODE.PAGE
          : t.isNil(validMatch)
          ? NAV_MODE.PRIMARY
        ...
        const checkBodyItem = t.not(t.eq(nextMode, NAV_MODE.PAGE))
          ? t.and(t.isNil(matchedBodyItem), t.not(t.isZeroLen(nextBody.items)))
