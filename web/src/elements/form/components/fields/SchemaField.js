...
function Help(props) {
  ...
  if (typeof help === 'string') {
    return <p className="help-block text-gray-600 text-xs italic">{help}</p>
  }
  return <div className="help-block text-gray-600 text-xs italic">{help}</div>
}
...
function ErrorList(props) {
  ...
  return (
    <div>
      <ul className="error-detail bs-callout bs-callout-info w-100">
        {errors
          .filter(elem => !!elem)
          .map((error, index) => {
            return (
              <li className="text-danger text-red-500 text-xs italic" key={index}>
                {error}
              </li>
            )
          })}
